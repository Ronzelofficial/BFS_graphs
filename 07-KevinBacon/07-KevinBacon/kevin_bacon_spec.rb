...
RSpec.describe Graph, type: Class do
  ...
  describe "finds correct path" do
    it "finds Kevin Bacon from himself" do
      expect(graph.bfs(kevin_bacon, kevin_bacon)).to eq("try again")
    end

    it "finds correct path" do
    ...
    it "finds correct path" do
      paths = ['Star Trek: Into Darkness',"Mission Impossible: Ghost Protocol", 'Guardians of the Galaxy', "Arrival",'Wanted', "Man of Steel",'X-Men: First Class']
      expect(graph.bfs(simon_pegg, kevin_bacon)).to eq(paths)
    end

    it "finds correct path" do
      paths = ['Mission Impossible: Ghost Protocol', 'Star Trek: Into Darkness', "Arrival", 'Guardians of the Galaxy',"Man of Steel", 'Wanted', 'X-Men: First Class']
      expect(graph.bfs(tom_cruise, kevin_bacon)).to eq(paths)
    end

    it "finds correct path" do
      paths = ['Arrival', "Man of Steel",'Mission Impossible: Ghost Protocol', 'Star Trek: Into Darkness', 'Guardians of the Galaxy', 'Wanted', 'X-Men: First Class']
      expect(graph.bfs(amy_adams, kevin_bacon)).to eq(paths)
    end

    it "finds correct path" do
      paths = ["Man of Steel", "Arrival", "Mission Impossible: Ghost Protocol", "Star Trek: Into Darkness", "Guardians of the Galaxy", "Wanted", "X-Men: First Class"]
      expect(graph.bfs(henry_cavill, kevin_bacon)).to eq(paths)
    end
